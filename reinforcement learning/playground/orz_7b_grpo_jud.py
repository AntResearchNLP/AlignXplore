...
from playground.zero_setting_base import CustomDataset, EvalCustomDataset, JudCustomDataset
...
class RewardModelTrainer(RayPPOTrainer):
    ...
    async def custom_reward_fn(
        ...
        assert len(tasks) == len(responsesA) == len(responsesB) == len(final_answers)
        ...
        preference_prompt = []
        for final_answer, task, responseA, responseB in zip(final_answers, tasks, responsesA, responsesB):
...
class GRPOExp(PPOExp):
    ...
    def train_dataset(self):
        ...
        prompts_dataset = JudCustomDataset(
            dialogues,
            self.tokenizer,
