...
class StreamingRayPPOTrainer:
    def __init__(
        ...
        self.reward_vllm_engines = reward_vllm_engines
    ...
    async def train(self):
        ...
        self.global_step = (consumed_samples // self.cfg.rollout_batch_size) * self.cfg.pref_steps
        ...
        for episode in range(start_episode, self.cfg.num_episodes):
            ...
            for iter, rand_prompts in enumerate(self.prompts_dataloader):
                ...
                pref_pbar = tqdm(
                    range(self.cfg.pref_steps), desc="Preference step"
                )
                for pref_step in range(self.cfg.pref_steps):
                    ...
                    if rand_prompts:
                        ...
                        all_prompts, all_extras, final_answers, acc_scores, sim_scores = await self.make_experience(rand_prompts)

                        if pref_step + 1 < self.cfg.pref_steps:
                            # get next round
                            unique_data = defaultdict(lambda: {'all_extras': [], 'final_answers': [], 'acc_scores': [], 'sim_scores': []})
                            for i, prompt in enumerate(all_prompts):
                                unique_data[prompt]['all_extras'].append(all_extras[i])
                                unique_data[prompt]['final_answers'].append(final_answers[i])
                                unique_data[prompt]['acc_scores'].append(acc_scores[i])
                                unique_data[prompt]['sim_scores'].append(sim_scores[i])

                            next_prompts, next_extras = [], []
                            for unique_data_item in unique_data.values():
                                unique_acc_scores = unique_data_item['acc_scores']
                                unique_sim_scores = unique_data_item['sim_scores']
                                unique_all_extras = unique_data_item['all_extras']
                                unique_final_answers = unique_data_item['final_answers']
                                
                                # random
                                selected_idx = random.randint(0, len(unique_final_answers) - 1)
                                
                                result_all_extra = unique_all_extras[selected_idx]
                                result_final_answer = unique_final_answers[selected_idx]

                                history = result_all_extra["history"]

                                cur_prompt = history[pref_step+1]['value']
                                cur_prompt = cur_prompt.replace("Not provided", result_final_answer)
...
"""
                                ...
                                prompt_instruction = prompt_instruction_template.render(prompt=cur_prompt)
                                prompt_template = Template(prompt_template_jinja)
                                if self.tokenizer.bos_token_id is None:
                                    bos_token = ""
                                else:
                                    bos_token = self.tokenizer.decode([self.tokenizer.bos_token_id])
                                cur_prompt = prompt_template.render(bos_token=bos_token, prompt=prompt_instruction)

                                next_prompts.append(cur_prompt)

                                next_extras.append(
                                    {"answer": history[pref_step+1]["answer"], "task": history[pref_step+1]["task"], "responseA": history[pref_step+1]["responseA"], "responseB": history[pref_step+1]["responseB"], "last_pref": result_final_answer, "history": history}
                                )
                            
                            assert len(next_prompts) == len(next_extras)

                            rand_prompts = []
                            if len(next_prompts) != 0:
                                rand_prompts = [(prompt, extra) for prompt, extra in zip(next_prompts, next_extras)]
    ...
    async def make_experience(self, all_inputs: Union[Tuple[str, dict], List[Tuple[str, dict]]], **generate_kwargs):
        ...
        if self.cfg.use_compute_reward_fn:
            async with Timer("Calculate custom rewards"):
                ...
                reward_fn = partial(self.custom_reward_fn, reward_model_fn=self._custom_reward_model_fn())
                all_prompts, outputs, custom_rewards, final_answers, acc_scores, sim_scores = await reward_fn(all_prompts, outputs, all_extras)
                assert len(all_prompts) == len(
                    outputs
                ), "generate objects number after custom reward function must be equal to all inputs number"
        else:
            all_prompts, outputs, custom_rewards, final_answers, acc_scores, sim_scores = all_prompts, outputs, None, [], [], [] 
        ...
        return all_prompts, all_extras, final_answers, acc_scores, sim_scores
    ...
    def _custom_reward_model_fn(self):
        raise NotImplementedError("custom reward model function is not supported yet")
    ...
    def _get_reward_generate_function(self, dp_rank: int):
        llm = self.reward_vllm_engines[dp_rank % len(self.reward_vllm_engines)]

        async def generate(prompts: List[str], truncate_prompt=True, **kwargs):
            if truncate_prompt:
                prompt_token_ids = self._reward_tokenize(prompts, self.cfg.prompt_max_len, padding=False)["input_ids"]
            else:
                prompt_token_ids = self._reward_tokenize(prompts, padding=False)["input_ids"]
            outputs = await llm.generate.remote(prompt_token_ids=prompt_token_ids, **kwargs)
            responses = []
            prompt_logprobs = []
            finish_reasons = []
    ...
    def _reward_tokenize(self, texts, max_length=99999999, padding=True, device=None):
    ...
    async def _offload_reward_vllm_engines(self):
        offload_tasks = []
        for engine in self.reward_vllm_engines:
            offload_tasks.append(engine.offload_to_cpu.remote())
        await asyncio.gather(*offload_tasks)
    
    async def _backload_reward_vllm_engines(self):
        backload_tasks = []
        for engine in self.reward_vllm_engines:
            backload_tasks.append(engine.backload_to_gpu.remote())
        await asyncio.gather(*backload_tasks)
    ...
    async def _sync_reward_weights_to_vllm(self):
        if self.cfg.colocate_all:
            for idx, reward_model in enumerate(self.reward_model):
                await self.reward_model[idx].async_run_method("_broadcast_to_vllm_cudaipc", self.reward_vllm_engines)
        else:
            for idx, reward_model in enumerate(self.reward_model):
                await self.reward_model[idx].async_run_method("_broadcast_to_vllm", self.reward_vllm_engines)
