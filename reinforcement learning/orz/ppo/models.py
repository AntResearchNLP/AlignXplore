...
class RewardActor(Actor):
    def forward(
        ...
        log_probs = probs_from_logits(output["logits"][:, :-1, :], sequences[:, 1:])
...
def _get_reward_model(base_pretrained_model, base_llm_model, value_head_prefix="value_head", packing_samples=False):
    ...
    class myRewardModel(base_pretrained_model):
        supports_gradient_checkpointing = True
        ...
        def forward(
            ...
            return (logits, outputs) if return_output else logits
